...
The Licensing tab has been removed from the Cloud Tiering interface. All licensing for Pay-as-you-go (PAYGO) subscriptions are accessed from the Cloud Tiering On-prem Dashboard now. There is also a link from that page to the Digital Wallet so that you can view and manage any Cloud Tiering bring-your-own-licenses (BYOL).
