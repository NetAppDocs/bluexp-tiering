...
Cloud Tiering is supported from single-node and high-availability AFF, FAS, and ONTAP Select clusters. Clusters in FabricPool Mirror configurations are also supported, but the configuration must be done using System Manager or the ONTAP CLI.
...
* IBM Cloud Object Storage (but the FabricPool configuration must be done using System Manager or the ONTAP CLI)
