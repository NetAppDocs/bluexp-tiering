...
Clusters in FabricPool Mirror configurations and MetroCluster configurations are also supported, but the tiering configuration must be done using System Manager or the ONTAP CLI.
...
In a typical configuration there is one object store for the entire cluster. Starting in August 2022, you can use the *Advanced Setup* page to add additional object stores for a cluster, and then attach different object stores to different aggregates.
...
* link:task-tiering-onprem-aws.html#preparing-amazon-s3[For Amazon, permissions are required to manage the S3 bucket].
