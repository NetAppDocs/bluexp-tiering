...
* If you've already subscribed to the BlueXP subscription (PAYGO) in your cloud provider's marketplace, then you're automatically subscribed to BlueXP Tiering for on-premises ONTAP systems as well. You'll see an active subscription in the BlueXP Tiering *On-prem dashboard* tab. You won't need to subscribe again. You'll see an active subscription in the Digital Wallet.
...
.Steps
[[subscribe-aws]]
. In BlueXP, click *Mobility > Tiering > On-Prem Dashboard*.
...
Bring-your-own licenses from NetApp provide 1-, 2-, or 3-year terms. The BYOL *BlueXP Tiering* license (previously known as a "Cloud Tiering" license) is a _floating_ license that you can use across multiple on-premises ONTAP clusters in your BlueXP account. The total tiering capacity defined in your BlueXP Tiering license is shared among *all* of your on-prem clusters, making initial licensing and renewal easy.
...
You use the Digital Wallet page in BlueXP to manage BlueXP Tiering BYOL licenses. You can add new licenses and update existing licenses.
...
* If you're using a configuration that's supported in BlueXP, your FabricPool licenses will be converted to BlueXP Tiering licenses and they'll appear in the Digital Wallet. When those initial licenses expire, you'll need to update the BlueXP Tiering licenses.
...
After you have purchased your BlueXP Tiering license, you activate the license in BlueXP by entering the BlueXP Tiering serial number and NSS account, or by uploading the NLF license file. The steps below show how to get the NLF license file if you plan to use that method.
...
When you link:task-managing-tiering.html#discovering-additional-clusters-from-bluexp-tiering[discover any of these special cluster types in BlueXP Tiering], BlueXP Tiering recognizes the FabricPool license and adds the license into the Digital Wallet. Those clusters will continue tiering data as usual. When the FabricPool license expires, you'll need to purchase a BlueXP Tiering license.
...
. The "parent" BlueXP Tiering license tracks the capacity being used for tiering by all clusters to make sure there is enough capacity in the license. The total licensed capacity and expiration date are shown in the Digital Wallet.
. A "child" tiering license is automatically installed on each cluster to communicate with the "parent" license.

NOTE: The licensed capacity and expiration date shown in System Manager or in the ONTAP CLI for the "child" license is not the real information, so don't be concerned if the information is not the same. These values are managed internally by the BlueXP Tiering software. The real information is tracked in the Digital Wallet.
...
. In BlueXP, link:task-licensing-cloud-tiering.html#add-bluexp-tiering-byol-licenses-to-your-account[add the license to the Digital Wallet].

. In BlueXP Tiering, link:task-managing-tiering.html#discovering-additional-clusters-from-blue-tiering[discover the new clusters].
