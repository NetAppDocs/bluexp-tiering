...
ONTAP clusters in the following configurations can use Cloud Tiering licenses, but they must be applied in a different manner than typical single-node and HA-configured ONTAP clusters:

* Tiering Mirror configurations
* MetroCluster configurations
* clusters that are tiered to IBM Cloud Object Storage

=== Process for existing clusters that have the FabricPool license installed

When you discover any of these clusters in Cloud Tiering, the FabricPool license is recognized and added to the Digital Wallet, and tiering continues as normal. When the FabricPool license expires, you'll need to purchase the new Cloud Tiering license.

=== Process for newly created clusters

Typically, when you configure an ONTAP cluster for Cloud Tiering using the Cloud Manager interface, the following actions happen:

. The "parent" Cloud Tiering license tracks the capacity being used for tiering by all clusters to make sure there is enough capacity in the license
. A "child" tiering license is automatically installed on each cluster to communicate with the "parent" license in the Digital Wallet

For the three configurations listed above, Cloud Tiering is configured on the cluster using System Manager or the ONTAP CLI. So in these cases you'll need to push the "child" license to these clusters manually from the Cloud Tiering interface.

TIP: Since data is tiered to two different object storage locations for Tiering Mirror and MetroCluster configurations, you'll need to purchase a license with enough capacity for tiering to both locations.

.Steps

. Install and configure your ONTAP clusters using System Manager or the ONTAP CLI. Do not configure tiering at this point.
...
. In Cloud Manager, link:task-licensing-cloud-tiering.html#add-cloud-tiering-byol-licenses-to-your-account[add the license to the Digital Wallet].
