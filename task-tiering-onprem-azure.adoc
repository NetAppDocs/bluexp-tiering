...
* An on-prem ONTAP cluster that's running ONTAP 9.4 or later and has an HTTPS connection to Azure Blob storage. link:task-discovering-ontap.html[Learn how to discover a cluster].
...
* link:task-installing-linux.html[Installing the Connector on an existing Linux host]
...
link:task-discovering-ontap.html[An on-premises working environment].
