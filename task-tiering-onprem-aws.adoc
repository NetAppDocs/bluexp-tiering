...
The Cloud Manager Connector is the main software for Cloud Manager functionality. A Connector is required to tier your inactive ONTAP data.
...
You need to discover your on-premises ONTAP cluster in Cloud Manager before you can start tiering cold data to object storage. You'll need to know the cluster management IP address and the password for the admin user account to add the cluster.
...
. Modify the security group configuration that's associated with the Cloud Manager Connector. You must change the policy to "Custom" (from "Full Access"), and you must <<Set up S3 permissions,add the required S3 Connector permissions>> as shown earlier.
+
...
+
The IPspace in the ONTAP cluster where the volumes you want to tier reside. The intercluster LIFs for this IPspace must have outbound internet access so that they can connect to your cloud provider's object storage.
+
...
+
A dialog box appears to help guide you through the endpoint configuration.
