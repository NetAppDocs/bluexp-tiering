...
Cloud Tiering licenses can also be shared with your clusters that are in MetroCluster and FabricPool Mirror configurations. The FabricPool configuration must be set up using System Manager or the ONTAP CLI, but https://docs.netapp.com/us-en/cloud-manager-tiering/task-licensing-cloud-tiering.html#apply-cloud-tiering-licenses-to-clusters-in-special-configurations[licensing for these types of clusters is completed using Cloud Tiering.]
