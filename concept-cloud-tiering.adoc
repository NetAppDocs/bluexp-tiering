...
Originally supported on AFF and FAS systems with all-SSD aggregates, starting with ONTAP 9.8 you can tier data from aggregates consisting of HDDs in addition to high-performance SSDs. See https://docs.netapp.com/us-en/ontap/fabricpool/requirements-concept.html[the considerations and requirements for using FabricPool^] for details.

TIP: Cloud Tiering is supported on AFF, FAS, and ONTAP Select systems.
