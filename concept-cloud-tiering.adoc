...
TIP: Cloud Volumes ONTAP systems are read-only from Cloud Tiering. link:task-tiering.html[You set up tiering for Cloud Volumes ONTAP from the working environment in Cloud Manager^].
...
* Cloud Backup is similarly affected by volumes set with a tiering policy. link:concept-backup-to-cloud.html#fabricpool-tiering-policy-considerations[See tiering policy considerations with Cloud Backup].
